...
ALTER TABLE products
ADD CONSTRAINT discontinued_zero_or_one
...
-- Отключаем ограничение внешнего ключа
ALTER TABLE order_details DROP CONSTRAINT order_details_product_id_fkey;

-- Удаляем все заказы, содержащие продукты со значением discontinued = 1
DELETE FROM order_details
WHERE product_id IN (SELECT product_id FROM products WHERE discontinued = 1);

-- Удаляем продукты со значением discontinued = 1
DELETE FROM products WHERE discontinued = 1;

-- Добавляем обратно ограничение внешнего ключа
ALTER TABLE order_details
ADD CONSTRAINT order_details_product_id_fkey FOREIGN KEY (product_id) REFERENCES products(product_id);
